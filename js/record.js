...
const Record = (() => {
  ...
  function renderModeSelect() {
    ...
    document.getElementById('rec-body').innerHTML = `
      <div class="section" style="margin-top:8px">
        <div style="font-size:14px;color:#888;text-align:center;margin-bottom:16px;padding:0 8px;line-height:1.6">
          오늘 일기를 어떻게 기록할까요?
        </div>
        <div style="display:flex;flex-direction:column;gap:10px">
          <button onclick="Record.startStep()" style="background:white;border-radius:16px;padding:20px 16px;text-align:left;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.06)">
            <div style="font-size:22px;margin-bottom:8px">💬</div>
            <div style="font-size:15px;font-weight:700;color:#222;margin-bottom:4px">질문형 대화</div>
            <div style="font-size:13px;color:#888;line-height:1.5">AI가 하나씩 질문하고,<br>순서대로 음성으로 답하는 방식</div>
          </button>
          <button onclick="Record.startFree()" style="background:white;border-radius:16px;padding:20px 16px;text-align:left;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.06)">
            <div style="font-size:22px;margin-bottom:8px">🎙</div>
            <div style="font-size:15px;font-weight:700;color:#222;margin-bottom:4px">한 번에 말하기</div>
            <div style="font-size:13px;color:#888;line-height:1.5">모든 질문이 한눈에 보이고,<br>자유롭게 한 번에 녹음하는 방식</div>
          </button>
        </div>
  ...
  function renderUploadLabel(type, icon, label) {
    ...
    return `<label style="display:flex;align-items:center;gap:10px;background:#f5f5f5;border-radius:10px;padding:11px 13px;cursor:pointer">
      ...
      <div style="flex:1"><div style="font-size:13px;font-weight:500;color:#222">${label}</div>
      <div style="font-size:11px;color:${isDone?'#2AADA3':'#999'}" id="${type}-status">${statusText}</div></div>
  ...
  function startFree() {
    ...
    const qList = questions.map((q,i)=>`
      <div style="display:flex;gap:8px;align-items:flex-start;padding:8px 0;border-bottom:0.5px solid #f0f0f0">
        <span style="font-size:11px;font-weight:600;color:#2AADA3;min-width:22px;margin-top:2px">Q${i+1}</span>
        <span style="font-size:13px;color:#444;line-height:1.5">${escapeHtml(q.text)}</span>
      </div>`).join('');

    document.getElementById('rec-body').innerHTML = `
      <div class="section">
        <div class="card" style="margin-bottom:4px">
          <div style="font-size:11px;font-weight:600;color:#888;margin-bottom:8px">오늘의 질문들</div>
          ${qList}
        </div>
        <div class="card">
          <div style="font-size:13px;color:#222;font-weight:600;margin-bottom:4px">🎙 위 질문들에 자유롭게 답해주세요</div>
          <div style="font-size:12px;color:#888;margin-bottom:14px;line-height:1.5">순서 상관없이, 생각나는 대로 말씀해 주세요.<br>AI가 알아서 일기로 정리해드려요.</div>
          <div class="voice-zone">
            ...
            <div class="stt-actions" id="stt-actions" style="display:none">
              ...
              <button class="btn-confirm" onclick="Record.confirmFree()">✓ 완료 · 일기 생성</button>
            </div>
          </div>
          <div class="divider-or">또는</div>
          <textarea class="text-input-area" id="text-answer" placeholder="자유롭게 오늘 하루를 써주세요..."
            style="height:120px" oninput="Record.onTextInput(this.value)"></textarea>
  ...
  function renderFinalStep() {
    ...
    document.getElementById('rec-body').innerHTML = `
      <div class="section">
        <div class="card">
          <div style="font-size:11px;font-weight:600;color:#888;margin-bottom:6px">선택 항목</div>
          <div style="padding:8px 0;border-bottom:0.5px solid #f0f0f0">
            ...
            <select id="child-time" style="font-size:13px;background:#f5f5f5;border:none;border-radius:8px;padding:8px 12px;width:100%;outline:none">
              <option value="">선택 안 함</option><option>거의 없음</option><option>1시간</option><option>2시간 이상</option>
            </select>
          </div>
  ...
  async function onGarminSelect(input, type) {
    ...
    reader.onload = async (e) => {
      ...
      if (statusEl) statusEl.textContent = '이미지 선택됨 · 분석 중...';
      ...
      if (result) {
        ...
        healthData = existing;
        if (statusEl) statusEl.textContent = type==='sleep'?`수면점수 ${result.sleepScore||'--'} 파싱완료`
  ...
  function startRecording() {
    ...
    recognition.onresult = (e) => {
      ...
      if (textEl) textEl.innerHTML = escapeHtml(finalTranscript) + (interim?`<span class="stt-interim"> ${escapeHtml(interim)}</span>`:'') + '<span class="stt-cursor"></span>';
  ...
  function stopRecording() {
    ...
    if (text) {
      ...
      const ta = document.getElementById('text-answer');
      if (ta) ta.value = text;
      if (recordMode === 'step') answers[currentStep].answer = text;
    } else {
  ...
  function confirmFree() {
    ...
    // 자유 발화를 첫 번째 답변으로 저장
    answers[0] = { question: '오늘 하루 자유 기록', answer: text };
    finalize();
  }

  function onTextInput(val) {
    if (recordMode === 'step' && currentStep < questions.length) answers[currentStep].answer = val;
  }
  ...
  return { init, next, skip, finalize, toggleVoice, retry, confirmVoice, confirmFree, onTextInput, onGarminSelect, startStep, startFree };
})();
